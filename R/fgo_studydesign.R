...
#' Study-Design Class
#'
#' @description subclass for representing the study design data. It inherits from FieldGo client class
...
#' @export
#'
FgStudyDesign <- R6::R6Class(

    classname = "FgStudyDesign",
    inherit = FieldGoClient,
    # private = list(
    #   super$endPoint = "/study-design/getAll?id="
    # ),

    public = list(

        #designId = NA, #designId
        ...
        fg_get_studydesign_studyId = function(studyDbId, format, ...){}#,#in BRAPI is location

    )
)


#' @title  Get experiment study design by studyID (studyDbId)
...
#' @export
#'
FgStudyDesign$set(which = "public", name = "fg_get_studydesign_studyId",
                function( studyDbId =NULL,
                          format=c("json","list","data.frame"),
                          ... ){

                    super$endPoint <- "/study/get?id="
                    url  <- paste0(self$serverURL, self$version, super$endPoint) #everything before the URL
                    print(url)

                    queryParams <- list(id = studyDbId)

                    #Iterate over design_id to extract experiment descript from each design in the study
